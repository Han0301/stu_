...
# ----------------------------------------------------------------set_params
save_path = "./model_pt/minst_best7.pt"
batch_size = 64
workers = 4
patience = 12
epochs = 100
...
class Conv_Model(torch.nn.Module):
    def __init__(self):
        ...
        self.linear = torch.nn.Linear(in_features=320, out_features=10)

    def forward(self, x):  # b*1*28*28
        batch_size = x.size(0)
        x = torch.nn.functional.relu(self.conv1(x))  # b*10*24*24
        x = self.maxpool(x)  # b*10*12*12
        x = torch.nn.functional.relu(self.conv2(x))  # b*20*8*8
        x = self.maxpool(x)  # b*20*4*4
        x = x.view(batch_size, -1)  # b*320
        return self.linear(x)  # b*10
...
if __name__ == "__main__":
    for epoch in range(epochs):
        ...
        save_plt(epochs_li, total_loss_li, "epoch", "total_loss", "loss with epoch", "./plt/minst_loss7_.png")
        save_plt(epochs_li, acc_li, "epoch", "acc", "acc with epoch", "./plt/minst_acc7_.png")
