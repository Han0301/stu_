...
**1. 本质理解** ⭐⭐
从输入图像到输出 3D 地图，整个视觉 SLAM Pipeline 中依次用到了哪些线性代数知识？（至少列出 5 个环节）

**2. 代码实践** ⭐⭐⭐
用 Python + NumPy 实现两帧重建函数 `two_view_reconstruction(kpts1, kpts2, K, matches)`，流程包括：归一化坐标 → 8 点法求本质矩阵 → 强制奇异值模式 → 恢复 R,t。

**3. 视觉关联** ⭐⭐
在 `two_view_reconstruction` 函数中，为什么最后要强制本质矩阵的奇异值模式为 `(σ, σ, 0)`？如果不做这一步会怎样？

**4. 原理追问** ⭐
从附录的 NumPy 速查表中，`np.linalg.lstsq` 和 `np.linalg.pinv` 在求解最小二乘时有何区别？各自适用于什么场景？
