...
**1. 本质理解** ⭐⭐⭐
指数映射和微积分中的"积分"有何类比关系？为什么说"李代数→李群"就像"速度→位移"？

**2. 概念辨析** ⭐⭐
Rodriguez 公式 `R = E + sinθ·n^∧ + (1-cosθ)·(n^∧)²` 的每一项分别对应什么几何意义？

**3. 代码实践** ⭐⭐⭐
手写 se(3) 的指数映射函数 `exp_se3(xi)`，输入 6 维向量 `[ρ, ϕ]`，输出 4×4 SE(3) 变换矩阵。要求正确处理 θ → 0 的情况。

**4. 视觉关联** ⭐⭐
小角度近似 `R ≈ E + ϕ^∧` 在 SLAM 优化中为什么如此重要？它和 BA 中的增量更新有什么关系？
