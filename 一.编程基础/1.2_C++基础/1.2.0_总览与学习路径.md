...
| 子专题 | 文件 | 掌握程度 |
|--------|------|:--------:|
| **指针与智能指针** | [1.2.1 →](1.2.1_指针与智能指针.md) | ⭐⭐ |
| **STL 容器** | [1.2.2 →](1.2.2_STL容器.md) | ⭐⭐ |
| **面向对象与多态** | [1.2.3 →](1.2.3_面向对象与多态.md) | ⭐⭐ |
| **CMake 工程构建** | [1.2.4 →](1.2.4_CMake工程构建.md) | ⭐⭐⭐ |
| **OpenCV C++ API** | [1.2.5 →](1.2.5_OpenCV_C++_API.md) | ⭐⭐ |
...
```mermaid
graph LR
    A["① 指针与引用<br/>1.2.1"] --> B["② STL 容器<br/>1.2.2"]
    B --> C["③ 面向对象<br/>1.2.3"]
    C --> D["④ CMake<br/>1.2.4"]
    B --> E["⑤ OpenCV C++<br/>1.2.5"]
```
// 引用传参——推荐方式（避免拷贝）
...
**1. 本质理解** ⭐⭐
为什么机器人视觉的"上线部署"环节必须用 C++？Python 在部署场景中有什么致命弱点？

**2. 代码实践** ⭐
写一个最小的 CMakeLists.txt，实现一个依赖 OpenCV 的可执行文件 `detection_node`。
